...
const BulkImport = ({ visible }) => {
	...
	const uploadMutation = useMutation((payload) => post('/suppliers/bulk', payload), {
		onSuccess: () => {
			message.success('Suppliers have been imported successfully');
			handleCancel();
	...
	const downloadSampleMutation = useMutation(() => get('/suppliers/sample', { responseType: 'blob' }), {
		onSuccess: (blob) => {
			Utils.downloadBlob(blob, 'sample_suppliers.csv');
