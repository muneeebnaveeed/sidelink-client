...
const BulkImport = ({ visible }) => {
	...
	const uploadMutation = useMutation((payload) => post('/employees/bulk', payload), {
		onSuccess: () => {
			message.success('Employees have been imported successfully');
			handleCancel();
	...
	const downloadSampleMutation = useMutation(() => get('/employees/sample', { responseType: 'blob' }), {
		onSuccess: (blob) => {
			Utils.downloadBlob(blob, 'sample_employees.csv');
