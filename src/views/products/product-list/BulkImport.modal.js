...
const BulkImport = ({ visible }) => {
	...
	const uploadMutation = useMutation((payload) => post('/products/bulk', payload), {
		onSuccess: () => {
			message.success('Products have been imported successfully');
			handleCancel();
	...
	const downloadSampleMutation = useMutation(() => get('/products/sample', { responseType: 'blob' }), {
		onSuccess: (blob) => {
			Utils.downloadBlob(blob, 'sample_products.csv');
